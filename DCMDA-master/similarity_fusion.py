...
def get_fusion_sim (k1, k2):
    ...
    sim_m1 = pd.read_csv("datasets/metabolite_GIP_similarity.csv", index_col=0, dtype=np.float32).to_numpy()
    sim_m2 = pd.read_csv("datasets/metabolites_information_entropy_similarity.csv", index_col=0, dtype=np.float32).to_numpy()
    sim_m3 = pd.read_csv("datasets/metabolites_structure_similarity.csv", index_col=0, dtype=np.float32).to_numpy()


    sim_d1 = pd.read_csv("datasets/disease_semantic_similarity.csv", index_col=0, dtype=np.float32).to_numpy()
    sim_d2 = pd.read_csv("datasets/disease_GIP_similarity.csv", index_col=0, dtype=np.float32).to_numpy()
    sim_d3 = pd.read_csv("datasets/disease _information_entropy_similarity .csv", index_col=0, dtype=np.float32).to_numpy()
